...
def run_yolo(img, conf=0.25):
    ...
    res = m(small, conf=conf, imgsz=640, verbose=False)[0]
    out = []
...
def annotate(img, dets):
    ...
    cv2.putText(img,"SkySentry AI",(10,img.shape[0]-10),
                cv2.FONT_HERSHEY_SIMPLEX,.5,(0,200,80),1)
    return img

@app.get("/")
def root(): return {"service":"SkySentry API","status":"online","docs":"/docs"}

@app.get("/health")
def health():
    m = _model is not None
    return {"status":"ok","model_loaded":m,"version":"3.0.0"}
...
async def detect_image(file: UploadFile = File(...)):
    ...
    if img is None:
        return JSONResponse({"error":"could not decode image"}, 400)
    dets = run_yolo(img, conf=0.20)   # Lower conf = more detections
...
async def detect_video(file: UploadFile = File(...), every_n: int = 5):
    import cv2
    data = await file.read()
    tmp  = f"/tmp/upload_{int(time.time())}.mp4"
    with open(tmp,"wb") as f: f.write(data)
    cap  = cv2.VideoCapture(tmp)
    if not cap.isOpened():
        return JSONResponse({"error":"could not open video"}, 400)
    ...
    try:
        while True:
            ...
            try:
                ...
                dets = run_yolo(img, conf=0.20)
                ann  = annotate(img.copy(), dets)
                await ws.send_text(json.dumps({
                    "count": len(dets),
                    "detections": dets,
                    "annotated": to_b64(ann),
...
class SD:
    ...
    def det(self):
        ...
        return {"bbox":[x1,y1,x2,y2],"conf":round(self.conf+random.uniform(-.03,.03),2),
                "class":self.dtype,"center":[int(self.x),int(self.y)]}
...
def intercept(ip,tp,tv):
    ...
    return {"heading_deg":round(math.degrees(math.atan2(-dy,dx))%360,1),
            ...
            "tti":round(tg,2),"feasible":SPD>math.hypot(tvx,tvy)*.85,
            "distance":round(d,1)}
...
class Session:
    ...
    def step(self):
        ...
        return {"frame":self.frame,"ts":int(time.time()*1000),
                "arena":{"w":W,"h":H},"drones":[d.det() for d in self.drones],
                "tracks":tgts,"interceptor":{"x":round(self.ix,1),"y":round(self.iy,1),"speed":SPD},
