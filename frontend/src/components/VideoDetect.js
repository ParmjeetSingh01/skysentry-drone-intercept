...
export default function VideoDetect(){
  ...
  return <div style={{display:'flex',flexDirection:'column',gap:20,fontFamily:M}}>
    <div onClick={()=>inp.current?.click()} style={{...box,border:'2px dashed rgba(0,255,100,0.25)',cursor:'pointer',textAlign:'center',padding:48}}>
      <div style={{fontSize:40,marginBottom:12}}>🎬</div>
      <div style={{color:'#00ff64',fontSize:13,letterSpacing:1}}>CLICK TO UPLOAD VIDEO</div>
    ...
    {loading&&<div style={{textAlign:'center',color:'#00ff64',letterSpacing:2,animation:'pulse 1s infinite'}}>⚙ {prog}</div>}
      ...
      <div style={{display:'flex',gap:12,flexWrap:'wrap'}}>
        {[{l:'File',v:result.filename},{l:'Total Frames',v:result.total_frames},{l:'Analyzed',v:result.analyzed_frames},{l:'FPS',v:result.fps?.toFixed(1)},{l:'Detections',v:result.total_detections}].map(({l,v})=>(
          <div key={l} style={{...box,flex:1,minWidth:100}}>
            <div style={lbl}>{l}</div>
            <div style={{color:'#00ff64',fontSize:14,fontWeight:'bold',marginTop:4}}>{v}</div>
          </div>
        ))}
      </div>
      <div>
        <div style={{...lbl,marginBottom:10}}>DETECTION TIMELINE</div>
        <div style={{background:'rgba(0,0,0,0.4)',borderRadius:4,padding:16,height:130,position:'relative',border:'1px solid rgba(0,255,100,0.08)'}}>
        ...
        <table style={{width:'100%',borderCollapse:'collapse'}}>
          ...
          <tbody>{result.frames.filter(f=>f.count>0).map((f,i)=>(
            <tr key={i} style={{borderBottom:'1px solid rgba(0,255,100,0.04)'}}>
              ...
              <td style={{color:'#00ff64',fontFamily:M,fontSize:11,padding:'4px 8px'}}>{f.timestamp_s}s</td>
              <td style={{color:f.count>2?'#ff3b30':'#00ff64',fontFamily:M,fontSize:12,padding:'4px 8px',fontWeight:'bold'}}>{f.count}</td>
