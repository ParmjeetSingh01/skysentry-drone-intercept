...
export default function WebcamDetect(){
  ...
  const threatColor=result?.count>0?'#ff3b30':'#00ff64';

  return <div style={{display:'flex',flexDirection:'column',gap:16,fontFamily:M}}>
    {/* Controls */}
    <div style={{display:'flex',gap:10,alignItems:'center',flexWrap:'wrap'}}>
      {!active
        ?<button onClick={start} style={{padding:'10px 28px',border:'1px solid #00ff64',background:'rgba(0,255,100,0.08)',color:'#00ff64',borderRadius:3,cursor:'pointer',fontFamily:M,fontSize:12,letterSpacing:2}}>▶ START WEBCAM</button>
      ...
      {[{l:'FPS',v:fps,alert:false},{l:'Live Detections',v:result?.count??0,alert:(result?.count??0)>0},{l:'Total Detected',v:total,alert:false}].map(({l,v,alert})=>(
        <div key={l} style={{padding:'8px 16px',border:'1px solid rgba(0,255,100,0.12)',borderRadius:3,background:'rgba(0,255,100,0.02)'}}>
          <div style={{fontSize:9,color:'rgba(0,255,100,0.4)',letterSpacing:2}}>{l}</div>
          <div style={{fontSize:15,color:alert?'#ff3b30':'#00ff64',fontFamily:M,fontWeight:'bold'}}>{v}</div>
        </div>
      ))}
    </div>
    ...
    {/* Video display */}
    <div style={{position:'relative',borderRadius:6,overflow:'hidden',border:'2px solid '+(active?threatColor:'rgba(0,255,100,0.15)'),background:'#030803',minHeight:360,display:'flex',alignItems:'center',justifyContent:'center'}}>
      ...
      {!active&&<div style={{textAlign:'center',color:'rgba(0,255,100,0.3)',letterSpacing:2,fontSize:13,padding:40}}>
      ...
      {active&&<div style={{position:'absolute',top:10,left:10,padding:'4px 12px',border:'1px solid '+(result?.count>0?'#ff3b30':'#34c759'),borderRadius:2,background:result?.count>0?'rgba(255,59,48,0.15)':'rgba(52,199,89,0.1)',color:result?.count>0?'#ff3b30':'#34c759',fontSize:11,letterSpacing:1,fontWeight:'bold',fontFamily:M,animation:result?.count>0?'pulse .8s infinite':'none'}}>
        {result?.count>0?result.count+' DRONE'+(result.count>1?'S':'')+' DETECTED':'NO THREATS'}
      </div>}
      {/* FPS badge */}
      {active&&<div style={{position:'absolute',top:10,right:10,padding:'4px 12px',border:'1px solid rgba(0,255,100,0.3)',borderRadius:2,background:'rgba(0,0,0,0.5)',color:'#00ff64',fontSize:11,fontFamily:M}}>
        {fps} FPS
      </div>}
    </div>

    {/* Detection list */}
    {result?.detections?.length>0&&<div style={{padding:14,border:'1px solid rgba(0,255,100,0.12)',borderRadius:4,background:'rgba(0,0,0,0.3)'}}>
      ...
      <table style={{width:'100%',borderCollapse:'collapse'}}>
        ...
        <tbody>{result.detections.map((d,i)=>(
          <tr key={i} style={{borderBottom:'1px solid rgba(0,255,100,0.04)'}}>
            <td style={{color:'#ff9f0a',fontFamily:M,fontSize:12,padding:'5px 8px',fontWeight:'bold'}}>{d.class?.toUpperCase()}</td>
            <td style={{color:'#00ff64',fontFamily:M,fontSize:12,padding:'5px 8px'}}>{(d.conf*100).toFixed(1)}%</td>
            <td style={{color:'#00d4ff',fontFamily:M,fontSize:11,padding:'5px 8px'}}>({d.center?.[0]}, {d.center?.[1]})</td>
            <td style={{color:'rgba(0,255,100,0.5)',fontFamily:M,fontSize:10,padding:'5px 8px'}}>{d.bbox?.join(', ')}</td>
          </tr>
        ))}</tbody>
      </table>
    </div>}
