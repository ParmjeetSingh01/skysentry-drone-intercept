...
export default function ImageDetect(){
  ...
  const box={padding:16,border:'1px solid rgba(0,255,100,0.12)',borderRadius:6,background:'rgba(0,255,100,0.02)'};
  const lbl={color:'rgba(0,255,100,0.45)',fontSize:10,letterSpacing:2,textTransform:'uppercase'};
  return <div style={{display:'flex',flexDirection:'column',gap:20,fontFamily:M}}>
    <div
      ...
      style={{...box,border:'2px dashed '+(drag?'#00ff64':'rgba(0,255,100,0.25)'),cursor:'pointer',textAlign:'center',padding:48,transition:'all .2s'}}>
      <div style={{fontSize:40,marginBottom:12}}>📁</div>
      <div style={{color:'#00ff64',fontSize:13,letterSpacing:1}}>DROP IMAGE OR CLICK TO UPLOAD</div>
    ...
    {loading&&<div style={{textAlign:'center',color:'#00ff64',letterSpacing:2,animation:'pulse 1s infinite'}}>⚙ RUNNING YOLO DETECTION...</div>}
      ...
      <div style={{display:'flex',gap:12,flexWrap:'wrap'}}>
        {[{l:'File',v:result.filename},{l:'Resolution',v:result.shape?result.shape[1]+'x'+result.shape[0]:'—'},{l:'Detections',v:result.count},{l:'Classes',v:[...new Set((result.detections||[]).map(d=>d.class))].join(', ')||'none'}].map(({l,v})=>(
          <div key={l} style={{...box,flex:1,minWidth:110}}>
            <div style={lbl}>{l}</div>
            <div style={{color:'#00ff64',fontSize:15,fontWeight:'bold',marginTop:4}}>{v}</div>
          </div>
        ))}
      </div>
      {result.annotated_b64&&<div>
        <div style={{...lbl,marginBottom:8}}>ANNOTATED OUTPUT</div>
        <img src={'data:image/jpeg;base64,'+result.annotated_b64} alt="annotated" style={{width:'100%',borderRadius:4,border:'1px solid rgba(0,255,100,0.15)'}}/>
      </div>}
        ...
        <table style={{width:'100%',borderCollapse:'collapse'}}>
          ...
          <tbody>{result.detections.map((d,i)=>(
            <tr key={i} style={{borderBottom:'1px solid rgba(0,255,100,0.04)'}}>
              <td style={{color:'rgba(0,255,100,0.5)',fontFamily:M,fontSize:11,padding:'5px 8px'}}>{i+1}</td>
              <td style={{color:'#ff9f0a',fontFamily:M,fontSize:11,padding:'5px 8px',fontWeight:'bold'}}>{d.class}</td>
              <td style={{color:'#00ff64',fontFamily:M,fontSize:11,padding:'5px 8px'}}>{(d.conf*100).toFixed(1)}%</td>
